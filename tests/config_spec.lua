...
T.test("config: defaults are complete without a setup call", function()
	...
	T.eq(config.get("notify"), true)
end)

T.test("config: a user table merges over the defaults", function()
	with_config({ path = "/opt/verifpal", panel = { focus = false } }, function(problems)
		T.eq(problems, {}, "no problems")
		T.eq(config.get("path"), "/opt/verifpal")
		T.eq(config.get("panel.focus"), false)
		T.eq(config.get("panel.height"), 20, "untouched keys keep their default")
	end)
end)
...
T.test("config: a nested misspelled option names its full path", function()
	with_config({ panel = { hieght = 1 } }, function(problems)
		T.eq(#problems, 1)
		T.matches(problems[1], "panel%.hieght")
	end)
end)
